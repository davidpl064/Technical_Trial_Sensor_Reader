...
class SensorInfrared():
    def __init__(self, min_value_range: int, max_value_range: int, uri_message_server: str):
        self._min_value_range = min_value_range
        self._max_value_range = max_value_range
        self._last_data = np.random.randint(min_value_range, max_value_range + 1, size=1, dtype=np.uint16)[0]
        self._uri_message_server = NatsUrl(url=uri_message_server)
        self._topic_raw_data = "sensors"
        self._freq_update_data = 1.
    ...
    async def connect_to_message_server(self):
        ...
        while not flag_connected:
            ...
            try:
                self.nats_client = await nats.connect(self._uri_message_server.url, connect_timeout=10, error_cb=self.connect_errors_handler)
                flag_connected = True
    ...
    def generate_data_mock(self):
        ...
        self._last_data = np.random.randint(self._min_value_range, self._max_value_range + 1, size=1, dtype=np.uint16)[0]
        logger.info(f"New sensor data generated: {self._last_data}")
    ...
    async def publish_data(self):
        await self.nats_client.publish(self._topic_raw_data, str(self._last_data).encode())